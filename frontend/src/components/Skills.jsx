      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {skillCategories.map((category, index) => (
            <div
              key={category.title}
              className="glass-panel p-8 rounded-3xl border border-border bg-surface/50 dark:bg-black/20 hover:border-accent-primary/20 transition-all duration-300 group"
            >
              ...
              <div className="flex flex-wrap gap-3">
                {category.skills.map(skill => (
                  <div 
                    key={skill.name}
                    className="flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-[var(--text-primary)] hover:border-accent-primary/30 transition-all duration-300 cursor-default"
                  >
